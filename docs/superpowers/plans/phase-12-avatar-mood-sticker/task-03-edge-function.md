# Task 03 — Edge Function: generate-mood-images

**Files:**

- Create: `supabase/functions/generate-mood-images/index.ts`

---

- [ ] **Step 1:** Verify chính xác model ID của Nano Banana trên OpenRouter.

Mở `https://openrouter.ai/collections/image-models`, tìm model Google Nano Banana, copy model ID (dạng `google/xxx`). Dùng ID đó trong bước 2.

- [ ] **Step 2:** Tạo `supabase/functions/generate-mood-images/index.ts`.
...
const OPENROUTER_MODEL = 'google/nano-banana'; // verify ID on openrouter.ai
...
const generateImage = async (
  ...
  const json = (await response.json()) as {
    choices: Array<{ message: { content: Array<{ type: string; image_url?: { url: string } }> } }>;
  };

  const imageContent = json.choices[0]?.message?.content?.find((c) => c.type === 'image_url');

  if (!imageContent?.image_url?.url) {
    throw new Error('No image in OpenRouter response');
  }

  return imageContent.image_url.url; // base64 data URL
};

const dataUrlToBlob = (dataUrl: string): Uint8Array => {
...
Deno.serve(async (request) => {
  ...
  // Get avatar signed URL from profiles
  ...
  try {
    // Generate all 5 stickers in parallel
...
```bash
npx supabase functions deploy generate-mood-images
```
...
```bash
git add supabase/functions/generate-mood-images/
git commit -m "feat: add generate-mood-images edge function"
```
