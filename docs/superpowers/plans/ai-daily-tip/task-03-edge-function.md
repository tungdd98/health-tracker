# Task 03 — Edge Function

**Goal:** Create the `generate-daily-tip` Supabase Edge Function. It is idempotent (returns cached row if today's tip already exists), fetches daily logs and assistant goals for personalization, calls Claude Haiku, and upserts the result into `daily_tips`.

**Files:**

- Create: `supabase/functions/generate-daily-tip/index.ts`

**Reference patterns:**

- Auth boilerplate: `supabase/functions/generate-mood-images/index.ts`
- Claude call pattern: `supabase/functions/chat-send/claude.ts` (but this task uses a simpler non-streaming call)
- Import header: must start with `import '../edge-runtime.d.ts';`

---

- [ ] **Step 1: Create `supabase/functions/generate-daily-tip/index.ts`**
...
- [ ] **Step 2: Verify the function can be deployed**

```bash
npx supabase functions deploy generate-daily-tip --no-verify-jwt
```

Wait — the function uses JWT auth internally, so `--no-verify-jwt` is NOT correct here. Deploy without that flag:

```bash
npx supabase functions deploy generate-daily-tip
```

Expected: `Deployed generate-daily-tip`

- [ ] **Step 3: Confirm `ANTHROPIC_API_KEY` secret is set**

The secret must already exist from the `chat-send` function. Verify:

```bash
npx supabase secrets list
```

Expected: `ANTHROPIC_API_KEY` appears in the list. If not, set it:
...
```bash
git add supabase/functions/generate-daily-tip/index.ts
git commit -m "feat(edge): add generate-daily-tip function"
```
