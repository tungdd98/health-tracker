...
**Critical UI implementation rule:** Do NOT write UI component code until the corresponding Pencil frame exists and is explicitly referenced in the plan task. Before touching any JSX or component markup, open the `.pen` file through the **Pencil MCP tool** and read the matching frame to get layout, spacing, and hierarchy. Reading `.pen` as raw JSON/text is not an acceptable substitute. If a required frame is missing, stop and design it before proceeding. UI implementation without a design frame reference is a process violation.
